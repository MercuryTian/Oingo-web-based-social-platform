function initMap() {
  var newyork = {lat:40.69694409291112,lng:-73.98769060707093};
  ...
  $.ajax({
    type: "post",
    url: "http://127.0.0.1:5000/notes/timeline",
    cache: false,
    async: false,
    dataType: "json",
    ...
    success: function (result) {
      ...
      console.log(result);
      var flag=window.location.href.split("=")[3];
        console.log(flag);
      //var flag=0；
        if (flag == 1){
          var markers = {
              ...
              nsid: 1,
              ntext: "A",
              ntime: "2018-12-11 10:21:50",
              tag: ["#DB"],
              uid: 1,
              username: "ZT",
              visibility: "all"
          }
            //result.notesList.push(markers);
            for (let i = 0, len = result.notesList.length; i < len; i++) {
                ...
                if (result.notesList[i].commentable == "1") {
                    contentstring += '<input type="text" name="comment" id="newcomment" value="" ">' +
                        '<br>' +
      ...
      for (let i = 0, len = result.notesList.length-1; i < len; i++) {
              // console.log(result.notesList[i]);
              markers[i] = new google.maps.Marker({
                  position: { lat: Number(result.notesList[i].nlati), lng: Number(result.notesList[i].nlongi) },
                  map: map,
...
function makecomment(noteid) {
  ...
  $.ajax({
    type: "post",
    url: "http://127.0.0.1:5000/notes/" + noteid.toString() + "/comment",
    dataType: "json",
...
function getlocation() {
  getPosition().then(result => {
    ...
    // $.ajax({
    //   type: "post",
    //   url: "http://127.0.0.1:5000/updateLocTime",
    //   cache:false, 
    //   async:false,
    //   dataType: "json",
